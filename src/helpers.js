...
const withDigest = obj => assoc([`internal`, `contentDigest`], digest(stringify(obj)), obj);
...
const createNodeHelpers = (options = {}) => {
  ...
  if (typeof options.sourceId !== `undefined` && typeof options.sourceId !== `string`)
    throw new Error(
  ...
  const generateNodeId = (type, id) => `${typePrefix}__${upperFirst(camelCase(type))}__${id}`;

  // Generates a node type name from a given type.
  const generateTypeName = type => upperFirst(camelCase(`${typePrefix} ${type}`));
  ...
  const createNodeFactory = (type, middleware = identity) => (obj, overrides = {}) => {
