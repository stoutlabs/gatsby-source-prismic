...
const isRichTextField = value =>
  Array.isArray(value) && typeof value[0] === "object" && Object.keys(value[0]).includes("spans");

// Returns true if the field value appears to be a Link field, false otherwise.
const isLinkField = value =>
  value !== null && typeof value === "object" && value.hasOwnProperty("link_type");
...
const isGroupField = value => Array.isArray(value) && typeof value[0] === "object";
...
const normalizeImageField = async args => {
  const { value, createNode, createNodeId, _auth, store, cache, touchNode } = args;
...
const normalizeGroupField = async args =>
  await map(args.value, async value => await normalizeFields({ ...args, value }));
